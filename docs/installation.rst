...
Clickhouse-driver supports Python 3.9 and newer and PyPy.
...
Packages for Linux and Mac OS X are available for python: 3.9 -- 3.14.

Packages for Windows are available for python: 3.9 -- 3.14.
...
The CityHash algorithm used for compressed block checksums is bundled with the
driver, so no extra package is required, see :ref:`compression-cityhash-notes`.

...
class InsertTestCase(BaseTestCase):
    ...
    def test_no_such_column(self):
        with self.create_table("a Float32"):
            with self.assertRaises(ServerException):
                data = [(1,)]
                self.client.execute("INSERT INTO test (b) VALUES", data)
    ...
    def test_empty_insert(self):
        with self.create_table("a Int8"):
            self.client.execute("INSERT INTO test (a) VALUES", [])
    ...
    def test_insert_return(self):
        with self.create_table("a Int8"):
            rv = self.client.execute("INSERT INTO test (a) VALUES", [])
    ...
    def test_profile_events(self):
        with self.create_table("x Int32"):
            data = [{"x": 1}]
            self.client.execute("INSERT INTO test (x) VALUES", data)
